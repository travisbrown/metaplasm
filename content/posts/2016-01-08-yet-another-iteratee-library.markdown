...
tags: scala, haskell, cats, iteratees
---
...
Eventually I started using more Scala than Haskell, but one of the things I always missed in Scala
...
API and lots of bugs—e.g. here's about the most basic thing you can imagine doing with `EnumeratorT.iterate`:
...
any monad you like as long as the monad you like is `scala.concurrent.Future`. I also just don't
prefer the way it mixes purity and impurity—e.g. I've gotten burned a couple of times because I
didn't know or remember that the enumerator you get from `Enumerator.enumerate` is side-effecting
and not reusable. It's still pretty good, though—I've chosen it fairly often given that I don't use
...
wrote [a little port of `Data.List.Split`][syzygist] for it, etc. I still find it extremely
complicated, though: I've repeatedly run into issues with e.g. [`gatherMap`][gather-map] and I still
don't have a clear sense of whether I was seeing bugs or misusing the library; the variance
gymnastics make my eyes bleed; etc.
...
take it as a starting point, so last month I published the port as an 0.1 release and set myself the
...
publishing a release as soon as Cats 0.4.0 is out.
...
2. Iteratees, which are computations that reduce streams of data to a value (think `sum`, `length`,
   `foldLeft`, etc.).
...
As a side note, stack safety is BYOB, so you generally don't want to use `cats.Id` as your monad
outside of simple examples like this. The library provides "modules" for several monads, including
`cats.Id` (which we get above from `io.iteratee.pure`), `cats.Eval` (which _is_ stack-safe), and
...
`core` and `task` subprojects of this library. `io.iteratee.task` provides `listFiles` and `lines`
enumerators that make this easy:
...
We don't have to worry about resource management because the `words` enumerator is doing that for
...
open files unnecessarily—if the first ten words that start with "e" are in the first file, then
...
magnitude of Scala's collection library. In the following benchmarks `I` stands for this library, `S` for
...
Stream, and scalaz-iteratee, all computations are in the `Task` monad. The results report
throughput, so higher numbers are better.
...
your stream library's combinators and the standard collection library operations for the sake of
...
For example, here's `Data.Iteratee`:
...
with bad iteratees. As part of this effort (and for the sake of performance), some of the work
...
The creation of iteratee.io was motivated in part by the need for streaming parsing in circe, and it follows the
[design guidelines][circe-design]. The 0.3.0 release of circe will include a [circe-streaming][circe-streaming]
module with a generic enumeratee for streaming JSON parsing. You can currently build this subproject yourself, and
there's a [circe tutorial][circe-sf-lots] with examples of how you can use it, but it's not yet released.
...
several new subprojects (for e.g. I/O operations in the standard library's `Future`).

Test coverage for the project is currently at 100%, in part thanks to [Discipline]-powered laws
checking, and my goal is to keep 100% coverage a requirement for at least the core project.
